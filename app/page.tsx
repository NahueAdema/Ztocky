...
const FEATURES: Feature[] = [
  {
    icon: Store,
    title: "Sabé qué se va a agotar",
    text: "Ztocky analiza tus ventas y te avisa con días de anticipación qué productos están por quedarse sin stock. Sin sorpresas.",
    accent: "bg-emerald-50 text-emerald-700",
  },
  {
    icon: TrendingDown,
    title: "Elegí al mejor proveedor",
    text: "Comprá informado: compará precios, tiempos de entrega y confiabilidad de todos tus proveedores en un solo lugar.",
    accent: "bg-sky-50 text-sky-700",
  },
  {
    icon: ShoppingCart,
    title: "Órdenes en un clic",
    text: "Generá órdenes de compra automáticas, revisalas, enviáselas a tu proveedor y recibí confirmación. Todo desde el mismo lugar.",
    accent: "bg-violet-50 text-violet-700",
  },
  {
    icon: BarChart3,
    title: "Consultá con IA",
    text: "Preguntale a Ztocky en español qué comprar, cuánto y cuándo. Sin planillas, sin cursos, sin complicaciones.",
    accent: "bg-amber-50 text-amber-700",
  },
];

const STATS: Stat[] = [
  { value: "–30%", label: "Menos productos agotados" },
  { value: "3x", label: "Más rápido al comprar" },
  { value: "100%", label: "Decisiones con datos reales" },
];

const BENEFITS = [
  "Alertas automáticas cuando un producto está por agotarse.",
  "Sugerencias de compra según el historial de tu negocio.",
  "Detección de productos que no se venden para liberar plata.",
  "Carga de precios de tus proveedores con solo pegar un CSV.",
];
...
function StatCard({ value, label }: Stat) {
  return (
    <div className="flex flex-col items-center gap-1 px-8 py-5 text-center">
      <span className="text-3xl font-bold tabular-nums text-primary sm:text-4xl">{value}</span>
      <span className="text-xs font-medium uppercase tracking-widest text-[#4B4B4C]/60">{label}</span>
    </div>
  );
}
      ...
      <div className="p-4">
        <div className="mb-4 grid grid-cols-3 gap-2">
          {[
            ...
            <div key={label} className="rounded-xl bg-[#f4f8f8] p-3 text-center">
              <p className="text-[10px] font-medium uppercase tracking-wider text-[#4B4B4C]/50">{label}</p>
        ...
        <div className="rounded-xl border border-border bg-white p-4">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-[#2e2e2f]">Café Brasil 1 kg</p>
          ...
          <div className="h-1.5 overflow-hidden rounded-full bg-[#e2f4f4]">
            <div className="h-full rounded-full bg-[#038786]" style={{ width: "28%" }} />
          </div>
        ...
        <div className="mt-3 flex items-start gap-2.5 rounded-xl bg-[#d9e9ec]/50 px-3.5 py-3">
          ...
          <p className="text-[11px] leading-5 text-[#4B4B4C]">
            <strong className="font-semibold">Recomendación:</strong> Comprá 200 unidades hoy para cubrir los próximos 18 días de venta y evitar que te quedes sin stock.
          </p>
        </div>
      </div>
    </div>
...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground antialiased">
      {/* ── HERO ───────────────────────────────────────────────────────────── */}
      <section id="inicio" className="relative overflow-hidden bg-[#d9e9ec] text-[#4B4B4C]">
        ...
        <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
          <Link href="/" className="flex items-center gap-2.5">
            <Image src="/logo.png" alt="Ztocky" width={160} height={60} priority className="h-15 w-auto rounded object-contain" />
          </Link>
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <Link href={mainHref} className="inline-flex h-10 items-center gap-2 rounded-full bg-gradient-to-r from-primary to-[#027978] px-5 text-sm font-semibold text-white shadow-lg shadow-primary/30 transition-all duration-200 hover:shadow-xl hover:shadow-primary/40 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
                {mainLabel}<ArrowRight className="h-3.5 w-3.5" />
              </Link>
            ) : (
              <>
                <Link href="/login" className="hidden sm:inline-flex h-10 items-center rounded-full border border-[#4B4B4C]/20 px-5 text-sm font-medium text-[#4B4B4C] transition-all duration-200 hover:bg-white/80 hover:border-[#4B4B4C]/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
                  Ingresar
                </Link>
                <Link href="/login?mode=register" className="inline-flex h-10 items-center gap-2 rounded-full bg-gradient-to-r from-primary to-[#027978] px-5 text-sm font-semibold text-white shadow-lg shadow-primary/30 transition-all duration-200 hover:shadow-xl hover:shadow-primary/40 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 gap-16 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-2 lg:items-center lg:py-32">
          <div className="flex flex-col items-start">
            <span className="mb-6 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-white/60 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-primary backdrop-blur-sm">
              ...
              Hecho para comercios de Latinoamérica
            </span>

            <h1 className="text-4xl font-extrabold uppercase leading-[1.08] tracking-tight text-[#2e2e2f] sm:text-5xl xl:text-6xl">
              Nunca más te quedes
              <br />
              <span className="text-primary">sin stock</span>
            </h1>

            <p className="mt-6 max-w-lg text-base leading-7 text-[#5f6062] sm:text-lg">
              Ztocky te avisa antes de que falte un producto, te dice a qué proveedor comprarle y genera la orden automáticamente. Todo en español y fácil de usar.
            </p>

            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Link href={mainHref} className="inline-flex h-11 items-center gap-2 rounded-full bg-primary px-7 text-sm font-semibold text-white shadow-lg shadow-primary/30 transition-colors duration-150 hover:bg-[#027978] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
                {user ? "Ir al dashboard" : "Comenzar gratis"}<ArrowRight className="h-4 w-4" aria-hidden />
              </Link>
              {!user && (
                <Link href="/login?mode=register" className="inline-flex h-11 items-center gap-2 rounded-full border border-[#4B4B4C]/20 bg-white/50 px-7 text-sm font-semibold text-[#4B4B4C] backdrop-blur-sm transition-colors duration-150 hover:bg-white/80">
                  Crear cuenta
                </Link>
              )}
            </div>
      ...
      {!user && (
        <div className="border-y border-border bg-white">
          <div className="mx-auto flex max-w-3xl flex-wrap items-stretch divide-x divide-border">
            {STATS.map((s) => (
              <div key={s.label} className="flex-1"><StatCard {...s} /></div>
            ))}
          </div>
        </div>
      )}
      ...
      <section id="funcionalidades" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-28">
        <div className="mb-12 max-w-2xl">
          ...
          <h2 className="mt-3 text-3xl font-bold text-[#2e2e2f] sm:text-4xl">
            Un sistema que te avisa
            <br />antes de que falte stock.
          </h2>
          <p className="mt-4 text-base leading-7 text-muted-foreground">
            Ztocky usa los datos de tu negocio para anticipar problemas y sugerirte la mejor decisión de compra.
          </p>
        </div>
      ...
      <section id="nosotros" className="border-y border-border bg-white">
        <div className="mx-auto grid max-w-7xl gap-14 px-5 py-20 sm:px-8 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div>
            <p className="text-sm font-semibold uppercase tracking-widest text-primary">Nosotros</p>
            <h2 className="mt-3 text-3xl font-bold text-[#2e2e2f] sm:text-4xl">
              Datos de tu negocio, decisiones inteligentes.
            </h2>
            <p className="mt-5 text-base leading-7 text-muted-foreground">
              Ztocky conecta tu historial de ventas con tus proveedores para decirte exactamente qué comprar, cuándo y a quién. Olvidate de las planillas y los cálculos a mano.
            </p>
            <Link href="/nosotros" className="mt-6 inline-flex h-10 items-center gap-2 rounded-full border border-primary/30 bg-white px-5 text-sm font-semibold text-primary transition-all duration-200 hover:bg-primary/5 hover:border-primary active:scale-[0.97]">
              Conocé más sobre Ztocky <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {BENEFITS.map((item) => (
              <div key={item} className="flex items-start gap-3 rounded-xl border border-border bg-background p-4">
                ...
                <p className="text-sm font-medium leading-6 text-[#2e2e2f]">{item}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div className="max-w-lg">
              ...
              <h2 className="mt-2 text-2xl font-bold text-[#2e2e2f] sm:text-3xl">
                Listo para dejar de perseguir el stock?
              </h2>
              <p className="mt-3 text-sm leading-6 text-[#5f6062]">
                Empezá hoy, sin configuraciones complicadas. En 5 minutos tenés todo listo.
              </p>
            </div>
            <div className="flex shrink-0 flex-col gap-3 sm:flex-row">
              <Link href={mainHref} className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-primary px-7 text-sm font-semibold text-white shadow-lg shadow-primary/25 transition-colors hover:bg-[#027978] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                <MailCheck className="h-4 w-4" aria-hidden />
                {user ? "Abrir dashboard" : "Ingresar a Ztocky"}
              </Link>
              {!user && (
                <Link href="/login?mode=register" className="inline-flex h-11 items-center justify-center gap-2 rounded-full border border-primary/25 bg-white/60 px-7 text-sm font-semibold text-[#4B4B4C] backdrop-blur-sm transition-colors hover:bg-white/90">
                  Crear cuenta gratis
                </Link>
              )}
            </div>
      ...
      <footer className="border-t border-border px-5 py-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-4 text-sm text-muted-foreground sm:flex-row sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <span>Ztocky — decisiones automáticas basadas en los datos de tu negocio</span>
          </div>
